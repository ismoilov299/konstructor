...
# Callback query xatoliklarini tuzatilgan YouTube bot kodi
...
class YouTubeBotHandler:
    ...
    async def process_download_callback(self, callback: CallbackQuery, state: FSMContext):
        """Download callback ni qayta ishlash - TUZATILGAN"""
        try:
            # MUHIM: Callback ni darhol answer qilish (timeout oldini olish)
            await callback.answer()
            ...
            # Darhol yuklab olish boshlanganini ko'rsatish
            await callback.message.edit_text(
                f"⚡ <b>Yuklab olish boshlandi!</b>\n\n"
                f"🎥 <b>{video_title[:50]}...</b>\n"
                f"🎯 <b>Format:</b> {format_choice}\n\n"
                f"⏳ <b>Jarayon:</b> Video tayyorlanmoqda...",
                parse_mode="HTML"
            )

            # Background taskda download qilish
            asyncio.create_task(
            ...
            # Callback allaqachon expire bo'lgan bo'lishi mumkin
            try:
                await callback.message.edit_text("❌ Yuklab olishda xatolik yuz berdi")
            except:
                # Agar message ni edit qila olmasak, yangi message yuboramiz
    ...
    async def _download_and_send_background(self, callback: CallbackQuery, video_url: str,
        ...
        """Background da download qilish (callback timeout muammosini hal qilish uchun)"""
        temp_dir = None
        try:
            ...
            if not progress_data or not progress_data.get('download_url'):
                await self._safe_edit_message(
                    callback.message,
                    "⏰ Video 3 daqiqada tayyor bo'lmadi.\n\n💡 Iltimos, keyinroq qayta urinib ko'ring."
                )
                return
            ...
            # Progress callback funksiyasi (xavfsiz)
            async def safe_progress_callback(percent, downloaded_bytes, total_bytes):
                try:
                    ...
                    # Message edit bo'lmasa ham davom etamiz
                    pass
            ...
            await self._safe_edit_message(callback.message, "❌ Yuklab olishda xatolik yuz berdi")
        finally:
    ...
    async def _safe_edit_message(self, message, text: str):
        """Xavfsiz message edit (callback timeout holatlari uchun)"""
        try:
            await message.edit_text(text, parse_mode="HTML")
        except Exception as e:
            # Agar edit bo'lmasa, yangi message yuboramiz
            try:
                await message.answer(text, parse_mode="HTML")
            except Exception as e2:
                self.logger.error(f"Could not send message: {e2}")

    async def _send_to_telegram_safe(self, callback: CallbackQuery, filepath: str,
                                     video_title: str, format_choice: str, file_ext: str):
        """Xavfsiz telegram ga yuborish"""
        try:
            ...
            try:
                ...
                await callback.message.edit_text("✅ Fayl muvaffaqiyatli yuborildi!")
            ...
            try:
                ...
                try:
                    ...
                    await callback.message.edit_text("✅ Fayl document sifatida yuborildi!")
            except Exception as doc_error:
                self.logger.error(f"Document send error: {doc_error}")
                await self._safe_edit_message(callback.message, "❌ Faylni yuborishda xatolik")


# Main handlers - TUZATILGAN VERSIYA
config = Config()
...
# MUHIM: Faqat bitta callback handler qoldirish
...
async def process_youtube_api_download(callback: CallbackQuery, state: FSMContext):
    """YouTube download callback handler - TUZATILGAN"""
...
async def cancel_download_callback(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Agar callback javob bera olmasak
        try:
